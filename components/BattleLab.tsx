...
export default function BattleLab({
  ...
  const [firstPlayerId, setFirstPlayerId] = useState(initialFirstId);
  const [secondPlayerId, setSecondPlayerId] = useState(initialSecondId);
  ...
  useEffect(() => {
    ...
    document.title = `${firstPlayer.name} vs ${secondPlayer.name} — Football Player Comparison | Tavalyze`;
  ...
  async function shareBattle() {
    const shareData = {
      title: `${firstPlayer.name} vs ${secondPlayer.name} — Tavalyze Battle`,
      text: `${firstPlayer.name} vs ${secondPlayer.name}: who wins? Compare their market value, age and Tavalyze Index.`,
  ...
  function surpriseMe() {
    const available = players.filter(
      (player) =>
        player.id !== firstPlayer.id && player.id !== secondPlayer.id,
    );
    const shuffled = [...available].sort(() => Math.random() - 0.5);

    setFirstPlayerId(shuffled[0]?.id ?? firstPlayer.id);
    setSecondPlayerId(shuffled[1]?.id ?? secondPlayer.id);
  }
      ...
      <section className="relative mx-auto max-w-7xl px-5 py-12 md:px-8 md:py-16">
        <header className="text-center">
          ...
          <button
            type="button"
            onClick={surpriseMe}
            className="mt-4 rounded-full border border-white/10 bg-white/[0.04] px-5 py-2.5 text-sm font-bold text-slate-300 transition hover:border-cyan-300/40 hover:bg-cyan-300/10 hover:text-cyan-200"
          >
            ✦ Surprise me
          </button>
        </header>
        ...
        <TrendingBattles
          currentFirstId={firstPlayer.id}
          currentSecondId={secondPlayer.id}
        />
...
function TrendingBattles({
  ...
  const battles = featuredBattles
    .map(([firstId, secondId]) => ({
      first: players.find((player) => player.id === firstId),
      second: players.find((player) => player.id === secondId),
    }))
    .filter(
      (battle): battle is { first: Player; second: Player } =>
        Boolean(battle.first && battle.second),
    );
    ...
    <section className="mt-12">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <h2 className="mt-2 text-3xl font-black">Pick the next showdown</h2>
        </div>
        <p className="text-sm text-slate-500">Made for debates. Built to share.</p>
      </div>

      <div className="mt-5 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {battles.map(({ first, second }) => {
          const active =
            first.id === currentFirstId && second.id === currentSecondId;

          return (
            <Link
              key={`${first.id}-${second.id}`}
              href={`/compare?first=${first.id}&second=${second.id}`}
              className={`rounded-2xl border p-4 transition hover:-translate-y-1 hover:border-cyan-300/40 ${
                active
                  ? "border-green-300/40 bg-green-300/10"
                  : "border-white/10 bg-white/[0.04]"
              }`}
            >
              <div className="flex items-center justify-between gap-2 text-xs font-black uppercase tracking-wider text-slate-500">
                <span>Battle</span>
                <span className="text-green-300">VS</span>
              </div>
              <p className="mt-3 font-black">{first.name}</p>
              <p className="my-1 text-xs font-bold text-slate-600">against</p>
              <p className="font-black">{second.name}</p>
            </Link>
          );
        })}
      </div>
